...
class BaseController:
    def __init__(self, arduino, base_frame, name='base_controller', node=None):
        self.inum = 0
    ...
    def setup_pid(self, pid_params):
        ...
        if self.arduino.update_pid(self.Kp, self.Kd, self.Ki, self.Ko):
            self.node.get_logger().info("PID parameters update to: Kp=%d, Kd=%d, Ki=%d, Ko=%d" %(self.Kp, self.Kd, self.Ki, self.Ko))
        else:
            self.node.get_logger().error("Updating PID parameters failed!")
    ...
    def poll(self):
        ...
        if now > self.t_next:
            ...
            if self.detect_enc_jump_error:
                try:
                    self.node.get_logger().info("Left: %d LEFT: %d Right: %d RIGHT: %d", left_enc, self.enc_left, right_enc, self.enc_right)
                    enc_jump_error = False
                    if abs(right_enc - self.enc_right) > self.enc_jump_error_threshold:
                        #self.diagnostics.errors += 1
                        self.bad_encoder_count += 1
                        self.node.get_logger().error("RIGHT encoder jump error from %d to %d", self.enc_right, right_enc)
                    ...
                    if abs(left_enc - self.enc_left) > self.enc_jump_error_threshold:
                        #self.diagnostics.errors += 1
                        self.bad_encoder_count += 1
                        self.node.get_logger().error("LEFT encoder jump error from %d to %d", self.enc_left, left_enc)
    ...
    def cmdVelCallback(self, req):
        ...
        #ctime = time.time()
        #self.inum += 1
        #print("base controller time:", ctime-self.btime, " ", self.inum)
